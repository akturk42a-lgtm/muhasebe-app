...
if st.button("PDF Bericht Generieren"):
    ...
    if res.data:
        ...
        # --- KASSEN BESTAND (Vortrag) - BELİRGİNLEŞTİRİLDİ ---
        pdf.set_font("Arial", "B", 11)
        pdf.set_fill_color(230, 230, 230) # Hafif gri arkaplan
        pdf.cell(130, 10, " KASSEN BESTAND (Vortrag aus dem Vormonat)", 1, 0, "L", True)
        pdf.cell(60, 10, f"{total_opening_balance:.2f} EUR ", 1, 1, "R", True)
        pdf.ln(5)

        # TABLO BAŞLIKLARI
        pdf.set_fill_color(200, 220, 255) # Mavi başlık
